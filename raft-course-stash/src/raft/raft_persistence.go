...
func (rf *Raft) readPersist(data []byte) {
	...
	// note: it will apply the snapshot here, so we need not to apply it again
	// by sending ApplyMsg to the applyCh
	rf.log.snapshot = rf.persister.ReadSnapshot()

	// important: if we restore snapshot, we shuold update the commit/lastApplied
	if rf.log.snapLastIdx > InvalidIndex {
